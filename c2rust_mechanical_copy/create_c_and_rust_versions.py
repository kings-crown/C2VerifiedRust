...
def _write_build_rs(rust_dir: str, program_name: str) -> None:
    """Create a build.rs that links the sibling C archive and extra libs."""
    c_dir = Path(rust_dir).parent / "c"
    build_rs_path = Path(rust_dir) / "build.rs"
    lines = [
        "use std::env;",
        ...
        f"    println!(\"cargo:rustc-link-lib=static={program_name}deps\");",
    ]
    for lib in _EXTRA_LIBS:
        lines.append(f"    println!(\"cargo:rustc-link-lib={lib}\");")
    lines += [
        "    // Also emit raw link args to force the flags through on some toolchains.",
        "    println!(\"cargo:rustc-link-arg=-L{}\", c_dir.display());",
        f"    println!(\"cargo:rustc-link-arg=-l{program_name}deps\");",
    ]
    for lib in _EXTRA_LIBS:
        lines.append(f"    println!(\"cargo:rustc-link-arg=-l{lib}\");")
    lines.append("}")
    build_rs_path.write_text("\n".join(lines))

def _ensure_cargo_uses_build_rs(rust_dir: str) -> None:
    cargo_path = Path(rust_dir) / "Cargo.toml"
    text = cargo_path.read_text()
    if 'build = "build.rs"' in text:
        return
    if "autobins = false" in text:
        text = text.replace("autobins = false", "autobins = false\nbuild = \"build.rs\"")
    else:
        ...
        for line in parts:
            ...
            if line.strip() == "[package]":
                in_pkg = True
                continue
            if in_pkg and line.startswith("[") and not inserted:
                new_lines.insert(len(new_lines) - 1, 'build = "build.rs"')
                inserted = True
                in_pkg = False
        if not inserted:
            new_lines.append('build = "build.rs"')
        text = "\n".join(new_lines)
...
def main():
    ...
    # Create a static archive of the C objects for linking the Rust crate.
    _run_command(f"cd \"{out_c_dir}\" && ar rcs lib{args['program_name']}deps.a *.o")
    ...
    unused_import_out, unused_import_retcode = _run_command(
        r"sed -i 's/^\(#!\[allow([^\)]*\))/\1, unused_imports)/'"
    ...
    # Ensure the Rust crate links the C archive and helper libs.
    _write_build_rs(out_rust_dir, args["program_name"])
    _ensure_cargo_uses_build_rs(out_rust_dir)
